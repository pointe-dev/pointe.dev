...
#   3. the qualify block spawns a pipeline             (hard assert),
#   4. the agent chain publishes a pitch               (hard assert).
#
...
# ── Hard assert 1: pipeline spawned ───────────────────────────────────────
if [ -z "$pipeline_id" ]; then
  echo "✗ FAIL: conversation never produced a qualify block / pipeline"; exit 1
fi
