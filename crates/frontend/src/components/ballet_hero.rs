...
//! deep dark field. Brand palette: red (#dc2626) lerped to teal (#00e5cc).
//!
//! Integration path (no winit): canvas → WebGL2 context → glow → three_d::Context,
//! driven by a requestAnimationFrame loop. A `paused` signal freezes the motion
//! (also honoured for `prefers-reduced-motion`, which starts paused), and the RAF
//! loop is torn down on unmount.
...
fn curve_point(u: f32) -> three_d::Vec3 {
...
pub fn BalletHero() -> impl IntoView {
    ...
    // Motion state. `paused` is user-facing (button); it also starts true when the
    // OS asks for reduced motion.
    let reduced = prefers_reduced_motion();
    let (paused, set_paused) = create_signal(reduced);

    create_effect(move |_| {
        let Some(canvas) = canvas_ref.get() else { return };
        let canvas: web_sys::HtmlCanvasElement = (*canvas).clone();

        // WebGL2 context from the Leptos-owned canvas (alpha so it overlays the
        ...
        // Build the particle curve as one instanced sphere mesh, gradient-coloured.
        let transformations: Vec<Mat4> = (0..POINTS)
            ...
            .collect();
        let colors: Vec<Srgba> = (0..POINTS)
            .map(|i| gradient(i as f32 / POINTS as f32))
            .collect();
        ...
        *cb.borrow_mut() = Some(Closure::wrap(Box::new(move |ts: f64| {
            ...
            // Advance the clock only when running, so pausing freezes the frame.
            ...
            // Size the drawing buffer to the displayed size × DPR.
        ...
        // Keep the closure alive for the component's lifetime; stop + drop on unmount.
        let keep = cb.clone();
        on_cleanup(move || {
            alive.set(false);
